...
def setup_cmd_input_complexes(multi, sequences, ordering, structure=''):
    """ Returns the command-line input string to be given to NUPACK. """
    if not multi:
        cmd_input = '+'.join(sequences) + '\n' + structure
    else:
...
def setup_nupack_input_complexes(**kargs):
    """ Returns the list of tokens specifying the command to be run in the pipe, and
    the command-line input to be given to NUPACK.
    Note that individual functions below may modify args or cmd_input depending on their
    specific usage specification. """
    ...
    # Set up command-line input to NUPACK
    cmd_input = setup_cmd_input_complexes(kargs['multi'], kargs['sequences'], kargs['ordering'],
                                kargs.get('structure', ''))
...
def call_with_file(args, cmd_input, outsuffix):
    ...
    os.remove(outfile.name[:-4]+'.ocx-key')
    os.remove(outfile.name[:-4]+'.prog')

    return output_lines

def call_with_file_concentrations(args, cmd_input, outsuffix):
    """ Performs a NUPACK call, returning the lines of the output in a temporary
    output file. The output file is assumed to have the suffix 'outsuffix'.
    outsuffix includes the period (.) delimiter.
      Ex:
    ...
    cmd_input = outprefix + '\n' + ''.join(cmd_input)
    #cmd_input2 = outprefix +'.con' + '\n' + str(1.0E-6) + '\n' + str(1.0E-6) +'\n' + str(1.0E-6)

    # prepend the output file prefix to the input for NUPACK
    #ao = [args[0], outprefix]
    p1 = sub.Popen(args, stdin=sub.PIPE, stdout=sub.PIPE, stderr=sub.STDOUT)
    output, error = p1.communicate(cmd_input)
    output_lines = output.decode().split('\n')

#    p2 = sub.Popen(args, stdin=sub.PIPE, stdout=sub.PIPE, stderr=sub.STDOUT)
#    p2.communicate(cmd_input2)
...
def call_with_pipe_concentrations(args, cmd_input):
    """ Performs a NUPACK call, returning the lines of the output from the pipe.
    """
    args = [str(s) for s in args]  # all argument elements must be strings
...
def complexes(sequences, ordering=None, material='dna',
          dangles='some', T=37, multi=True, pseudo=False,
          sodium=1.0, magnesium=0.0):
    """Calls NUPACK's pfunc on a complex consisting of the unique strands in sequences, returns dG.
         sequences is a list of the strand sequences
    ...
    ## Set up command-line arguments and input
    args, cmd_input = \
        setup_nupack_input_complexes(exec_name='complexes', sequences=sequences, ordering=ordering,
                           material=material, sodium=sodium, magnesium=magnesium,
                           dangles=dangles, T=T, multi=multi, pseudo=pseudo)
    suffix = '.ocx'

    ## Perform call
    output_complex = call_with_file(args, cmd_input, suffix)
    ## Parse and return output

    pair_probs = []
    for l in filter(lambda x: x[0].isdigit(), output_complex):
        if len(l.split()) > 1:
            i, j, p, a, b, c = l.split()
            pair_probs.append((int(i), int(j), int(p), int(a), int(b), float(c)))

    return pair_probs

def concentrations(args):
